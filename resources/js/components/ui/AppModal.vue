<template>
    <Teleport to="body">
        <div v-if="modelValue" class="fixed inset-0 z-50 grid place-items-center p-4" @keydown.esc="close">
            <button type="button" class="absolute inset-0 bg-slate-950/55 backdrop-blur-sm" @click="close"></button>
            ...
            >
                <header class="flex items-center justify-between border-b border-sky-100 px-5 py-4">
                    ...
                    <button type="button" class="rounded-lg px-2 py-1 text-slate-500 transition hover:bg-sky-50" @click="close">x</button>
                </header>
                ...
                <footer v-if="$slots.footer" class="border-t border-sky-100 px-5 py-4">
                    <slot name="footer" />
                </footer>
            </section>
        </div>
