<template>
    <section class="space-y-4">
        ...
        <div class="flex flex-wrap items-end gap-3">
            ...
            <select v-model.number="messageFilters.per_page" class="rounded-lg border border-slate-300 px-3 py-2 text-sm" @change="onMessagePerPageChange">
                <option :value="10">10 / page</option>
                <option :value="15">15 / page</option>
                <option :value="30">30 / page</option>
            </select>
            <button class="rounded-lg bg-slate-900 text-white px-4 py-2 text-sm font-semibold" @click="refreshInbox">Refresh</button>
        ...
        <article class="rounded-2xl border border-slate-200 bg-white overflow-x-auto">
            ...
            <footer class="flex flex-wrap items-center justify-between gap-2 border-t border-slate-200 px-4 py-3 text-xs text-slate-600">
                <p>Page {{ messagePagination.page }} of {{ messagePagination.last_page }} | {{ messagePagination.total }} messages</p>
                <div class="flex gap-2">
                    <button class="rounded-lg border border-slate-300 px-3 py-1 font-semibold text-slate-700 disabled:opacity-50" :disabled="isMessagePrevDisabled" @click="prevMessagePage">Prev</button>
                    <button class="rounded-lg border border-slate-300 px-3 py-1 font-semibold text-slate-700 disabled:opacity-50" :disabled="isMessageNextDisabled" @click="nextMessagePage">Next</button>
                </div>
            </footer>
        </article>
...
async function load() {
    try {
        const response = await MessageService.inbox({
            ...
            page: messageFilters.value.page,
            per_page: messageFilters.value.per_page,
        });

        rows.value = response.data.data ?? [];
        unreadCount.value = Number(response.data.unread_count ?? 0);
        syncPagination(messagePagination.value, response.data, messageFilters.value.per_page);
        messageFilters.value.page = messagePagination.value.page;
    } catch (error) {
...
const isMessagePrevDisabled = computed(() => (messagePagination.value.page ?? 1) <= 1);
const isMessageNextDisabled = computed(() => (messagePagination.value.page ?? 1) >= (messagePagination.value.last_page ?? 1));

async function refreshInbox() {
    messageFilters.value.page = 1;
    await load();
}

async function onMessagePerPageChange() {
    messageFilters.value.page = 1;
    await load();
}

async function prevMessagePage() {
    if (isMessagePrevDisabled.value) {
        return;
    }
...
async function nextMessagePage() {
    if (isMessageNextDisabled.value) {
        return;
    }
...
function syncPagination(target, payload, fallbackPerPage = 15) {
