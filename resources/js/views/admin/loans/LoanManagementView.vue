    ...
    <section class="space-y-5">
        ...
        <div class="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
            <article class="rounded-2xl border border-slate-200 bg-white p-4">
                ...
                <p class="mt-2 text-2xl font-black text-slate-900">{{ loanPagination.total }}</p>
            </article>
        ...
        <article class="rounded-2xl border border-slate-200 bg-white p-4">
            <div class="flex flex-wrap items-end justify-between gap-3">
                ...
                <div class="flex flex-wrap items-center gap-2">
                    <select v-model.number="loanFilters.per_page" class="rounded-xl border border-slate-300 px-3 py-2.5 text-sm" @change="onLoanPerPageChange">
        ...
        <article class="overflow-x-auto rounded-2xl border border-slate-200 bg-white">
            ...
            <footer class="flex flex-wrap items-center justify-between gap-2 border-t border-slate-200 px-5 py-3 text-xs text-slate-600">
                <p>Page {{ loanPagination.page }} of {{ loanPagination.last_page }} | {{ loanPagination.total }} loans</p>
                <div class="flex gap-2">
                    <button class="rounded-lg border border-slate-300 px-3 py-1 font-semibold text-slate-700 disabled:opacity-50" :disabled="isLoanPrevDisabled" @click="prevLoanPage">Prev</button>
                    <button class="rounded-lg border border-slate-300 px-3 py-1 font-semibold text-slate-700 disabled:opacity-50" :disabled="isLoanNextDisabled" @click="nextLoanPage">Next</button>
                </div>
            </footer>
        </article>
...
async function load() {
    try {
        const response = await LoanService.adminList({
            ...
            page: loanFilters.page,
            per_page: loanFilters.per_page,
        });

        rows.value = response.data.data ?? [];
        syncPagination(loanPagination.value, response.data, loanFilters.per_page);
        loanFilters.page = loanPagination.value.page;
    } catch (error) {
...
const isLoanPrevDisabled = computed(() => (loanPagination.value.page ?? 1) <= 1);
const isLoanNextDisabled = computed(() => (loanPagination.value.page ?? 1) >= (loanPagination.value.last_page ?? 1));

async function onLoanPerPageChange() {
    loanFilters.page = 1;
    await load();
}

async function prevLoanPage() {
    if (isLoanPrevDisabled.value) {
        return;
    }
...
async function nextLoanPage() {
    if (isLoanNextDisabled.value) {
        return;
    }
