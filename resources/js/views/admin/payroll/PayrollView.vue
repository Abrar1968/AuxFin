                ...
                <tbody>
                    <tr v-for="row in rows" :key="row.id" class="border-t border-slate-100 hover:bg-slate-50/70">
                        ...
                        <td class="space-x-3 p-3 text-right">
                            ...
                            <button
                                class="text-xs font-semibold text-rose-700 hover:text-rose-900 disabled:cursor-not-allowed disabled:opacity-50"
                                :disabled="String(row.status).toLowerCase() === 'paid'"
                                @click="removePayroll(row.id)"
                            >
                                Delete
                            </button>
                        </td>
...
async function removePayroll(id) {
    ...
    try {
        ...
        toast.error(getApiErrorMessage(error, 'Unable to delete payroll record.'));
    }
}
