    ...
    <section class="space-y-5">
        ...
        <div class="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
            <article class="rounded-2xl border border-slate-200 bg-white p-4">
                ...
                <p class="mt-2 text-2xl font-black text-slate-900">{{ messagePagination.total }}</p>
            </article>
        ...
        <article class="rounded-2xl border border-slate-200 bg-white p-4">
            ...
            <div>
                ...
                <div class="flex flex-wrap gap-2">
                    <select v-model.number="messageFilters.per_page" class="rounded-xl border border-slate-300 bg-white px-3 py-2.5 text-sm" @change="onMessagePerPageChange">
...
async function load() {
    try {
        const response = await MessageService.adminList({
            ...
            per_page: messageFilters.per_page,
        });

        rows.value = response.data.data ?? [];
        unreadCount.value = Number(response.data.unread_count ?? 0);
        syncPagination(messagePagination.value, response.data, messageFilters.per_page);
        messageFilters.page = messagePagination.value.page;
    } catch (error) {
