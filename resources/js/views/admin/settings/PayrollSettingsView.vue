        ...
        <article class="rounded-2xl border border-slate-200 bg-white p-5">
            <div class="flex flex-wrap items-center justify-between gap-3">
                ...
                <div class="flex items-center gap-2 text-xs text-slate-600">
                    <span>Per page</span>
                    <select v-model.number="holidayFilters.per_page" class="rounded-lg border border-slate-300 px-2 py-1" @change="onHolidayPerPageChange">
            ...
            <div class="mt-3 flex flex-wrap items-center justify-between gap-2 text-xs text-slate-600">
                <p>Page {{ holidayPagination.page }} of {{ holidayPagination.last_page }} | {{ holidayPagination.total }} holidays</p>
                <div class="flex gap-2">
                    <button class="rounded-lg border border-slate-300 px-3 py-1 font-semibold text-slate-700 disabled:opacity-50" :disabled="isHolidayPrevDisabled" @click="prevHolidayPage">Prev</button>
                    <button class="rounded-lg border border-slate-300 px-3 py-1 font-semibold text-slate-700 disabled:opacity-50" :disabled="isHolidayNextDisabled" @click="nextHolidayPage">Next</button>
                </div>
            </div>
        </article>
    </section>
...
async function loadHolidays() {
    try {
        const response = await SettingsService.holidays({
            ...
            per_page: holidayFilters.per_page,
        });

        holidays.value = response.data.data ?? [];
        syncPagination(holidayPagination.value, response.data, holidayFilters.per_page);
        holidayFilters.page = holidayPagination.value.page;
    } catch (error) {
...
const isHolidayPrevDisabled = computed(() => (holidayPagination.value.page ?? 1) <= 1);
const isHolidayNextDisabled = computed(() => (holidayPagination.value.page ?? 1) >= (holidayPagination.value.last_page ?? 1));

async function onHolidayPerPageChange() {
    holidayFilters.page = 1;
    await loadHolidays();
}

async function prevHolidayPage() {
    if (isHolidayPrevDisabled.value) {
        return;
    }
...
async function nextHolidayPage() {
    if (isHolidayNextDisabled.value) {
        return;
    }
...
function syncPagination(target, payload, fallbackPerPage = 25) {
