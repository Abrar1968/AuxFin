    ...
    <section class="space-y-5">
        ...
        <div class="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
            <article class="rounded-2xl border border-slate-200 bg-white p-4">
                ...
                <p class="mt-2 text-2xl font-black text-slate-900">{{ leavePagination.total }}</p>
            </article>
        ...
        <article class="rounded-2xl border border-slate-200 bg-white p-4">
            ...
            <div class="flex flex-wrap items-center gap-2">
                <select v-model.number="leaveFilters.per_page" class="rounded-xl border border-slate-300 px-3 py-2.5 text-sm" @change="onLeavePerPageChange">
        ...
        <article class="overflow-x-auto rounded-2xl border border-slate-200 bg-white">
            ...
            <footer class="flex flex-wrap items-center justify-between gap-2 border-t border-slate-200 px-5 py-3 text-xs text-slate-600">
                <p>Page {{ leavePagination.page }} of {{ leavePagination.last_page }} | {{ leavePagination.total }} leave requests</p>
                <div class="flex gap-2">
                    <button class="rounded-lg border border-slate-300 px-3 py-1 font-semibold text-slate-700 disabled:opacity-50" :disabled="isLeavePrevDisabled" @click="prevLeavePage">Prev</button>
                    <button class="rounded-lg border border-slate-300 px-3 py-1 font-semibold text-slate-700 disabled:opacity-50" :disabled="isLeaveNextDisabled" @click="nextLeavePage">Next</button>
                </div>
            </footer>
        </article>
...
async function load() {
    try {
        const response = await LeaveService.adminList({
            status: status.value || undefined,
            page: leaveFilters.page,
            per_page: leaveFilters.per_page,
        });

        rows.value = response.data.data ?? [];
        syncPagination(leavePagination.value, response.data, leaveFilters.per_page);
        leaveFilters.page = leavePagination.value.page;
    } catch (error) {
...
const isLeavePrevDisabled = computed(() => (leavePagination.value.page ?? 1) <= 1);
const isLeaveNextDisabled = computed(() => (leavePagination.value.page ?? 1) >= (leavePagination.value.last_page ?? 1));

async function onLeavePerPageChange() {
    leaveFilters.page = 1;
    await load();
}

async function prevLeavePage() {
    if (isLeavePrevDisabled.value) {
        return;
    }
...
async function nextLeavePage() {
    if (isLeaveNextDisabled.value) {
        return;
    }
